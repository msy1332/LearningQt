...
/*
    ...
    第一种方法就是将这个QWidget这个窗口当一个独立的窗口来显示，也就是通过QWidget的shoe这个方法就可以将这个窗口独立的显示出来
    然后的话就是第二种方法，就是将这个QWidget这个窗口当作别的窗口的子类，也就是将这个窗口内嵌到别的窗口
    那怎么内嵌呢，方法其实也很简单，也就是将别的窗口作为自己的父类就可以实现了，首先就是在创建这个窗口类的时候将别的窗口的指针传进来就可以将别的窗口作为自己的父类了，并且啊如果你是指针的话，父类也会帮你自动的释放内存，并且啊你不需要显示的调用show方法来显示出来，因为这个窗口是父类的这个窗口的内嵌窗口所有的话当父类的窗口显示的出来的时候，子类的窗口也会显示出来，并且啊这个窗口是内嵌到这个父类这个窗口里面的
